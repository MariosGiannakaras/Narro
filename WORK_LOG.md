...
- Scraped 	emp_app residue from package.json, package-lock.json, index.html, and focus.html.
...
### Validation performed
- \npm run build -> PASS. Vite generated index.html and focus.html\ successfully.
...
### TODO/STATUS updates
- Reverted M1 scaffolding checkboxes to [ ] except for the Vite frontend split (focusSurface bundle), which was structurally verified via \npm run build\.
...
- Added explicit focus_surface_show, _hide, _focus commands and exposed them in the React harness.
- Updated Mode switching commands (focus_surface_mode_panel and _timer) to ensure the window is shown.
...
### Validation performed
- npm ci, npm run build, cargo check --locked, cargo test --locked -> PASS in CI.
- npm run tauri build -> PASS in CI, producing NSIS artifact.
...
### Results
- main -> focusSurface shared Rust state: **PASS**
- focusSurface -> main shared Rust state: **PASS**
- Hide/Show Main: **PASS**
- Destroy Main keeps Rust process and focusSurface alive: **PASS**
- Rust state still mutates while main is destroyed: **PASS**
- Recreate Main: **FAIL** (Hangs focusSurface and opens a blank white window).
...
### Changed
- Converted `main_window_recreate` in `src-tauri/src/lib.rs` to an `async fn`. The synchronous Tauri 2 command directly calling `WebviewWindowBuilder::build()` deadlocked the WebView2 initialization thread on Windows, completely hanging the app.
...
### Decisions
- Moving to `async fn` for window creation matches Tauri 2's explicit Windows-specific guidance for avoiding IPC/Event loop deadlocks. 
...
### Exact continuation point
- The user must download the NEW `narro-m1-runtime-harness-windows-x64` artifact (including the raw `narro.exe`) and re-run the `Recreate Main` scenario to confirm the deadlock is solved.

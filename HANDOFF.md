...
Current validated **source** implementation baseline is PR #32 squash merge `349260f28475f53472b444af6180704a4b981c20`. Windows PR CI #161 / run `33936979665`: SUCCESS. Windows main CI #162 / run `33947484856`: SUCCESS.
...
Highest-confidence M3 lesson: tracked-time loss is a recurring Blitzit failure family from late 2024 through current 2026 reports. Narro now has explicit automated coverage for completion-to-zero prevention, pause/resume accounting, restart recovery, task-switch restart, post-pause manual Time Taken rebasing, and renderer-independent typed timer/session projection. Do not remove these persistence-first boundaries when adding notification/prompt integration.
...
Continue from the validated typed-event timer reliability baseline. Do not start Milestone 4 or polished product UI.

Priority order:

1. Complete Pomodoro authoritative boundary side effects and late-observation correctness. `TimerEngine::advance_inner` can currently cross Work -> Break -> Paused in one sufficiently late call, while `TimerRuntime::commit_candidate` compares only initial/final binding; therefore the next slice must persist and publish every crossed Work/Break boundary in order so an intermediate Break session cannot disappear.
2. Emit Pomodoro break-start and break-finish notification **decisions** once per authoritative boundary, including late observation and recovery cases, then submit the corresponding Windows notifications. Do not claim impossible transactional exactly-once external toast delivery; explicitly distinguish authoritative once-only boundary decision from OS submission failure/crash semantics.
3. Implement durable user-visible end-of-Pomodoro-break awaiting-resume state on top of the typed projection so both `main` and `focusSurface`, including recreated webviews, can show the prompt and Resume can issue the existing authoritative `timer_resume` command.
4. Add long-duration/large-elapsed safety coverage so timer/session arithmetic and event revisioning cannot overflow or corrupt state.
5. Define/test Windows sleep/resume behavior for **no data loss**. Do not invent whether unattended sleep counts as work; that accounting policy requires an explicit product decision before implementation.
6. Use exact-head Windows CI as the authoritative Rust/frontend/release gate.

Suggested next source branch: `ai/m3-pomodoro-boundary-effects`, based on current `main` after tracking reconciliation.

Do not pull Milestone 4 scheduling/recurrence materialization into M3.

## USER ACTION REQUIRED

**None for the current Pomodoro backend/typed-prompt slice.** Physical Windows validation is only needed for visible notification delivery behavior after the automated semantics are correct; the generic Windows notification capability itself was already physically proven in M1.

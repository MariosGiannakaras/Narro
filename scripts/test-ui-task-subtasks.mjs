...
for (const [haystack, needle, label] of [
  ...
  [listBoardRust, "subtask_counts(conn, task.id)?", "board subtask count read"],
  [listBoardRust, "assert_eq!(board.today.tasks[0].subtask_total_count, 2);", "board subtask total regression"],
  [listBoardRust, "assert_eq!(board.today.tasks[0].subtask_completed_count, 1);", "board subtask completion regression"],
  ...
  [board, "mutable: Boolean(taskSubtaskPanel.snapshot?.mutable)", "authoritative mutable projection in All Lists"],
  ...
  [css, "width: 6.25rem;", "fixed parent action slot with destructive action"],

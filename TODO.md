...
- [ ] Implement Focus Panel <-> Floating Timer content transition with short one-shot opacity/transform motion; do not animate native window geometry in a high-frequency JS loop.
  ...
  - [ ] Native hidden-resize corrective candidate: PR #125 exact head `a652116dacda255bcb22a551ee6750504c72bc6a`; CI #481 failed only the Rust formatting gate, the evidence-backed rustfmt-only correction is pushed, and exact-head Windows CI #482 is running. Do not count this as validated until exact-head CI, guarded merge and resulting-main CI pass.

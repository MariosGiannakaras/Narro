...
- [ ] Implement Focus Panel <-> Floating Timer content transition with short one-shot opacity/transform motion; do not animate native window geometry in a high-frequency JS loop.
  ...
  - [x] CI #507 follow-up: settled expand/collapse and session continuity again passed, but one 20 fps Panel→Timer capture showed blank/pale staging frames before Timer content rendered. Treat the continuous no-flash criterion as FAIL pending reproduction and root-cause analysis; see `work-log/2026-09-25-codex-m7-ci507-physical-followup.md`.

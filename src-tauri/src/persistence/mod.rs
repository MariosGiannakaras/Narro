use rusqlite::Connection;
use rusqlite_migration::{Migrations, M};

pub fn run_migrations(conn: &mut Connection) -> Result<(), rusqlite_migration::Error> {
    let migrations = Migrations::new(vec![M::up(include_str!(
        "../../migrations/0001_initial.sql"
    ))]);

    migrations.to_latest(conn)
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_migrations_fresh_and_repeated() {
        let mut conn = Connection::open_in_memory().expect("open in-memory database");

        run_migrations(&mut conn).expect("fresh migration should succeed");

        {
            let mut stmt = conn
                .prepare(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='_diagnostic_startup'",
                )
                .expect("prepare table existence query");
            let exists = stmt.exists([]).expect("query migration-created table");
            assert!(
                exists,
                "Table _diagnostic_startup should exist after migration"
            );
        }

        run_migrations(&mut conn).expect("repeated migration should succeed without errors");
    }
}

...
The prior #140 source and #138 DOM fix were physically exercised as detailed above. The #143 executable physically passed basic shortcut use and both-chord conflict/retry, while two continuous Panel→Timer captures failed the no-staging-flash criterion. Fix that result with a coherent transition state-machine change, then complete OS reduced motion, hidden Timer, unavailable monitor/taskbar/DPI recovery, and independent borderless/exclusive fullscreen per `docs/M7_FLOATING_RUNTIME_VALIDATION.md`. Avoid repeating completed resource runs without a performance-relevant source change. Do not advance to Milestone 8.

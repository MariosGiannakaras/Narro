...
GitHub Actions `Windows CI` is the reproducible compile/test/package gate when the agent environment cannot run native Windows UI. It covers locked frontend build, repository/config verification, Rust fmt/check/Clippy/tests, the deterministic floating-performance harness self-test, Tauri Windows release build and diagnostic artifact upload.
...
Narro uses official `tauri-plugin-autostart` 2.5.1 through Narro-owned Rust commands only: `autostart_status`, `autostart_enable`, and `autostart_disable`. Enable/disable is caller-idempotent, the requested state is queried again after mutation, and query/enable/disable/state-mismatch failures use structured `CommandError` codes. No guest `autostart:*` permission surface was exposed.
...
Physical enable/disable plus actual next-sign-in/reboot launch remains MANUAL NOT RUN.

Evidence: `work-log/2026-09-03-chatgpt-windows-autostart.md`.

## Floating-only performance measurement state
...
- full repository preflight: PASS;
- Tauri release build: PASS;
- artifact upload: PASS;

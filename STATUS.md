...
M7 items 1–6 and 13–14 are validated. The earlier CI #480/#501 physical expand/collapse failures are superseded by the physical CI #503 re-test: three complete native resize cycles, including expanded subtask controls, showed no retained/duplicated action strip in the collapsed DOM or pixels. Normal-size horizontal overflow and timer/session continuity passed. A #507 20 fps Panel→Timer capture showed blank/pale staging frames before Timer content rendered; the no-flash part of item 7 is **FAIL pending reproduction/root-cause analysis**, and OS reduced motion remains untested. #507 basic T/P and both-chord ownership conflict/retry received scoped physical PASS. Hidden Timer P on #507, display-topology/DPI, and independent fullscreen cases remain open; see `work-log/2026-09-25-codex-m7-ci507-physical-followup.md`. Item 14's final-UI physical resource protocol completed on CI #505: three valid idle runs per state, each with zero churn, plus a separate running sample. Idle CPU medians were 0.000% of one core in both states; running average was 0.155%. Memory and caveats are in the earlier work log. No M8 work has started.
...
The prior #140 source and #138 DOM fix were physically exercised as detailed above. The #143 executable physically passed basic shortcut use and both-chord conflict/retry, while one continuous Panel→Timer capture failed the no-staging-flash criterion. Reproduce and investigate that result, then complete OS reduced motion, hidden Timer on #507, unavailable monitor/taskbar/DPI recovery, and independent borderless/exclusive fullscreen per `docs/M7_FLOATING_RUNTIME_VALIDATION.md`. Avoid repeating completed resource runs without a performance-relevant source change. Do not advance to Milestone 8.

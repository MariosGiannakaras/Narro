...
The user-provided 60 fps physical recording of CI #514 is a scoped **FAIL** for M7 item 7. With normal Windows animations, Panel→Timer briefly exposes target staging states before the authoritative task projection settles. The recording then visibly switches Windows 10 `Show animations in Windows` to **Off**; in that OS-reduced-motion state, transitions around ~32.8s and ~34.3s expose several blank white target-host frames before Panel content appears. This confirms that a hidden WebView presented-frame barrier alone does not guarantee a composited target frame.

PR #147 applies the evidence-backed correction at the native host boundary. After hidden geometry preparation and synchronous React target publication, Narro temporarily enables Win32 layered-window alpha and shows the real `focusSurface` at alpha 0. The existing finite two-`requestAnimationFrame` barrier therefore runs while WebView2 is actually visible and able to paint; reveal uncloaks the same HWND and only then publishes native Panel/Timer mode authority. Failure recovery prewarms and restores the previous presentation through the same path. No fixed delay, polling loop, extra webview, renderer timer/session authority, or high-frequency geometry loop was introduced.

PR #147 exact head `8f173cd37fc8d1506ec546feb2248e92db81cebe`, tree `ab122091235c2f3720df74ede84152238020cfd9`, passed Windows CI #516 / run `36186563007`. Expected-head guarded squash merge produced resulting main source `f59e4d16a49659832ea562c3718cc5ba748b74fb` with the identical tree. Resulting-main Windows CI #517 / run `36188196900` / job `108246665399` passed Repository Preflight, visual regression, Tauri Release and both required artifact uploads.
- #517 runtime artifact `10886794088`, digest `sha256:644ca2135ec313334da862778869a7d1cd5771b62840bc43fbd4aeadaa67dbb5`;
- #517 visual artifact `10887670005`, digest `sha256:6a553510abc8ee15100e419d89378b86c441365cef8dd2b6bce6d8d57d3e6982`.

The conflicting broader PR #146 was closed as superseded after #147 merged. Automated validation still does **not** close item 7. Exact next action is a physical 60 fps retest from the #517 runtime artifact, first with normal Windows animations and then with the actual OS setting Off. Require no blank/pale/staging frame, no abrupt return flicker, no horizontal scrollbar, and no stale expanded pixels. If that passes, continue the remaining M7 hidden-Timer, secondary-monitor/topology/no-saved-position, taskbar/DPI/constrained-work-area, and borderless/exclusive-fullscreen matrix. Do not advance to Milestone 8.

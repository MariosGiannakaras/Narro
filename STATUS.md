...
The reproduced Panel↔Timer staging-flash failure now has an automated-validated source correction. PR #145 exact head `f901fa907b550e921daf32123ec84a49aebb7006` passed Windows CI #513 / run `36173387766`: repository preflight, visual regression, Tauri Release and both artifact uploads all succeeded. Expected-head guarded squash merge produced validated source commit `c875b4894e90cc75c04c9ff9508ef1dc1a176ad5`, tree `67ced4d48545772a48b5452f686f3b15127ce5a2` byte-identical to the exact PR-head tree. Resulting-main Windows CI #514 / run `36176196311` also passed all required gates.

The corrective transition coordinator splits native hidden preparation from reveal, synchronously publishes the target React hierarchy while the host remains hidden, waits for the shared presented-frame barrier, and only then reveals the native surface. Timer and Panel mode authority is recorded only on reveal. Failure/cancellation after target preparation rolls back through the same prepare → publish → frame → reveal sequence; Panel native rollback restores size, position, topmost, taskbar and visibility. CI #514 runtime artifact `10882911552` digest `sha256:b73a1dafd98af88eb1f76886508dd07786005e121fd5f001205085eccf1f483e`; visual artifact `10883280101` digest `sha256:226331d647043da70f7ee3c7af9a489da38e0407060eb3cfd8c1e5a252cebd13`.

Automated validation does **not** close item 7. Exact next action is physical Windows re-validation from the CI #514 runtime artifact: continuous Panel→Timer and Timer→Panel capture with no blank/staging frame, plus Windows animations Off/reduced-motion follow-up. Then complete the remaining hidden-Timer, secondary-monitor/topology/no-saved-position, taskbar/DPI/constrained-work-area, and borderless/exclusive-fullscreen checks in `docs/M7_FLOATING_RUNTIME_VALIDATION.md`. Do not advance to Milestone 8.
